...
int GeneralizedAssignemnt::fixSol(int *infeasSol, int *zsol) {
    ...
    for (i = 0; i < m; i++)
        capres.push_back(cap[i]);
    /***************** 1个block，n个thread，无需共享内存；初始化赋值 **********************************/
    for (i = 0; i < n; i++)
        sol.push_back(infeasSol[i]);
...
int GeneralizedAssignemnt::fixSolViaKnap(int *infeasSol, int *zsol) {
    ...
    *zsol = INT_MAX;
    ...
    /*********** 1个block，n个thread，无需共享内存；初始化赋值 *******************************************/
    for (j = 0; j < nelem; j++)
        val[j] = 1; // any element could be chosen (val > 0)
    nreset = 0;
    ...
    if (nreset < nelem)
        goto lend; // could not recover fesibility
...
double KDynRecur(int n, int Kcap, int *Q, double *val, int *Ksol) {
    ...
    // 根据res反推路径，仅由0号线程进行
    KdecodeSol(n - 1, imax, Q, val, n, f, Ksol);
